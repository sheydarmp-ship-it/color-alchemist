...
impl Menu {
    ...
    fn draw_mode(&self) {
        ...
        let single = if self.selected == 0 { GREEN } else { WHITE };

        let multi = if self.selected == 1 { GREEN } else { WHITE };
        let leader = if self.selected == 2 { GREEN } else { WHITE };
    ...
    fn draw_difficulty(&self) {
        ...
        let easy_color = if self.selected == 0 { GREEN } else { WHITE };

        let medium_color = if self.selected == 1 { GREEN } else { WHITE };

        let hard_color = if self.selected == 2 { GREEN } else { WHITE };
    ...
    fn draw_network(&self) {
        let host = if self.selected == 0 { GREEN } else { WHITE };

        let join = if self.selected == 1 { GREEN } else { WHITE };
        ...
        draw_text("UP/DOWN : Select", 100.0, 400.0, 28.0, LIGHTGRAY);

        draw_text("ENTER : Continue", 100.0, 440.0, 28.0, LIGHTGRAY);
    }
    fn draw_ready(&self) {
        draw_text("Ready to Start!", 150.0, 140.0, 45.0, GREEN);

        let player = format!("Player : {}", self.player_name);
        ...
        let diff = format!("Difficulty : {}", difficulty);
    ...
    fn draw_leaderboard(&self) {
        ...
        for (i, player) in save.players.iter().enumerate() {
            let total = player.easy + player.medium + player.hard;
